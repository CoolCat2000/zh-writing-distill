---
name: distill-humanized
description: 从当前目录 docs 中的中文文章蒸馏个人写作风格，并在 AGENTS.md 中合入去 AI 生成痕迹规则。用于用户调用 /distill-humanized，要求提炼文风并去 AI 味、人味化、反 AI 腔、自然化改写，或让 Codex、Claude Code、Gemini、OpenCode 等 Agent 按个人风格写得更像真人时。
user-invocable: true
license: MIT
...
把当前目录 `docs/` 下的中文文章蒸馏成可复用的个人写作规则，并额外加入去 AI 生成痕迹规则，写入当前目录的 `AGENTS.md`。`CLAUDE.md` 始终作为指向 `AGENTS.md` 的软链接存在，方便 Codex、Claude Code、Gemini、OpenCode 等 Agent 共用同一份规则。

当用户调用 `/distill-humanized`，或要求提炼个人写作风格并同时去除 AI 痕迹、人味化、反 AI 腔、自然化改写、减少 ChatGPT 味时，执行本 Skill。

如果用户只要求普通文风蒸馏且没有提到 AI 痕迹，改用 `distill` skill。
...
- 去 AI 味必须服务于作者声音；不要把文字改成另一种“更口语”的模板。

## 执行流程

1. 检查当前目录是否存在 `docs/`，并确认其中至少有一个 `.md`、`.mdx` 或 `.txt` 文件。
2. 读取 `references/style_distillation.md`，按其中方法分析素材。
3. 读取 `references/ai_trace_removal.md`，把去 AI 味规则合入生成内容。
4. 生成完整的中文 `AGENTS.md` 内容。
5. 按操作系统使用本 Skill 自带的 `scripts/write_memory_files.sh` 或 `scripts/write_memory_files.ps1` 写入文件并创建软链接，不要手写备份和软链逻辑。
...
“去 AI 生成痕迹”部分必须来自 `references/ai_trace_removal.md`，并结合作者素材调整，不要只复制通用禁词表。
